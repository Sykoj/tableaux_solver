...
namespace tableaux {
	...
	tokens formula_parser::get_formula_tokens(const std::string& unparsed_formula) {
		...
		while (it != end) {
			...
			auto current_token = token::try_get_token_and_iterate_over(it, end);
	...
	formula_ptr formula_parser::assemble_formula(const tokens& tokens) {
		...
		if (tokens.size() == 1 && tokens[0].get_type() == token::type::variable) return get_variable(tokens[0]);
		if (tokens.size() == 1 && tokens[0].get_type() == token::type::subformula) return get_subformula(tokens[0]);
	...
	formula_ptr formula_parser::get_binary_formula(const tokens& tokens) {
		...
		if (tokens[0].get_type() == token::type::subformula) lhs = get_subformula(tokens[0]);
		else if (tokens[0].get_type() == token::type::variable) lhs = get_variable(tokens[0]);
		if (tokens[2].get_type() == token::type::subformula) rhs = get_subformula(tokens[2]);
		else if (tokens[2].get_type() == token::type::variable) rhs = get_variable(tokens[2]);
	...
	formula_ptr formula_parser::get_unary_formula(const tokens& tokens) {
		...
		if (tokens[0].get_type() == token::type::junction && tokens[0].get_value() == "NOT") {
			
			if (tokens[1].get_type() == token::type::subformula) subformula = get_subformula(tokens[1]);
			else if (tokens[1].get_type() == token::type::variable) subformula = get_variable(tokens[1]);
	...
	formula_ptr formula_parser::get_subformula(const token& token) {

		return parse_formula(token.get_value());
	}

	formula_ptr formula_parser::get_variable(const token& token) {

		return std::make_unique<formula_variable>(token.get_value()[0]);
	}
